...
export default function WorkCard({ item, selected, onSelect }) {
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "group flex h-14 min-w-[120px] shrink-0 cursor-pointer snap-start items-center gap-3 overflow-hidden rounded-full border px-4 py-2 text-left transition-all duration-300 md:min-w-0 md:flex-1 md:basis-0",
        selected
